...
const Me = async (req, res, next) => {
	...
	try {
		const user = await User.findById(user_id).select("-password -__v"); //parola ve versiyon bilgisi çıkartıldı.
...
const AllUser = async (req, res, next) => {
	...
	try {
		const users = await User.find({}).select("-password -__v"); //parola ve versiyon bilgisi çıkartıldı.
